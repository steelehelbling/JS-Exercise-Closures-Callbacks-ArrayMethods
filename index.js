...
function processDuplicateFree(list, callback) {
  ...
  let process = list.filter(function (item1, item2) { list.indexOf(item1) === item2});
...
function firstNamesAllCaps(runners) {
  /* CODE HERE */
  return runners.map(function(item){return item.first_name.toUpperCase()})
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  /* CODE HERE */
  return runners.filter(function (item) {return item.shirt_size == tShirtSize})
  /* CODE HERE */
}
...
function tallyUpDonations(runners) {
  /* CODE HERE */
  return runners.reduce((acc, value) => {
